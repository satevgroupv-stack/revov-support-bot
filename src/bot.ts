...
const texts = {
  en: {
    ...
    instantFix_paymentFailed: "Your payment failed. Please try again or use another payment method.",
    instantFix_outOfStock: "This item is out of stock. Please select another product.",
    error: "Something went wrong. Please try again.",
    done: "✅ Your report has been sent to our support team (24/7). We'll follow up.",
    langSelected: (lang: string) => `Language set to ${lang}. Let's continue.`,
  ...
  am: {
    ...
    instantFix_paymentFailed: "ክፍያዎ አልተሳካም። እባክዎ እንደገና ይሞክሩ ወይም ሌላ የክፍያ ዘዴ ይጠቀሙ።",
    ...
    langSelected: (lang: string) => `ቋንቋ ወደ ${lang} ተቀየረ። እንቀጥል።`,
...
function classifyIssue(description: string, product: string): string {
  ...
  if (lowerDesc.includes('payment') || lowerDesc.includes('pay') || lowerDesc.includes('failed') || lowerDesc.includes('lakipay'))
    return 'payment error';
  if (lowerDesc.includes('out of stock') || lowerDesc.includes('not available') || lowerDesc.includes('empty'))
    return 'out of stock';
  if (lowerDesc.includes('network') || lowerDesc.includes('wifi') || lowerDesc.includes('connection'))
    return 'network issue';
  if (lowerDesc.includes('electrical') || lowerDesc.includes('interference') || lowerDesc.includes('grounding'))
    return 'electrical interference';
  if (lowerDesc.includes('dispense') || lowerDesc.includes('stuck') || lowerDesc.includes('not coming out'))
    return 'dispensing failure';
  if (lowerDesc.includes('machine') || lowerDesc.includes('frozen') || lowerDesc.includes('crash'))
    return 'machine malfunction';
  return 'other';
}

// ---------- Forward to support group ----------
async function forwardToSupportGroup(ctx: MyContext, category: string, instantFixGiven: boolean) {
  const summary = `
📢 NEW ISSUE REPORT
